...
const customBikesStationMarker = L.Marker.extend({
  options: {
    id: 0
  }
})

const customBikesLayer = L.Layer.extend({

})

const bikesStationPopupOptons = {
  // 'maxWidth': '500',
...
function getBikesIcon (d_) {
  ...
  const two = 'images/transport/bikes_icon_blue_2.png'
  const three = 'images/transport/bikes_icon_blue_3.png'
  const four = 'images/transport/bikes_icon_blue_4.png'
  const five = 'images/transport/bikes_icon_blue_5.png'
  ...
  d3.json('/api/dublinbikes/stations/' + sid_ + '/today')
    .then(function (stationData) {
      ...
      if (stationData[0].banking) {
        ...
        const bankStr = '<img alt="Banking icon " src = "images/bank-card-w.svg" height= "20px" title="Banking available" />'
      ...
      const processedData = stationData[0].historic.map((d) => {
...
function setBikeStationColour (bikes, totalStands) {
  const percentageFree = (bikes / totalStands) * 100
