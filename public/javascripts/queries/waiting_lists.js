...
//    d3.select('.viz1').
    ...
    const areaChartContainerWidth = Math.floor(d3.select(".viz1").style("width").slice(0, -2));
    const areaChartContainerHeight = Math.floor(d3.select(".viz1").style("height").slice(0, -2));
    console.log("w: ", areaChartContainerWidth);
    console.log("h: ", areaChartContainerHeight);
    var timeLine = dc.lineChart("#waiting-list-chart");
//    var timeLineWidth = areaChartContainerWidth;
    timeLine
            .width(areaChartContainerWidth).height(areaChartContainerHeight)
            .brushOn(false).renderArea(true).xyTipsOn(true)
//            .margins({top: 50, right: 150, bottom: 50, left: 50})
            .dimension(dateDim)
...
//                    interactive legend for each stack
    const itemWidth = 500;
    timeLine.legend(dc.legend().x(areaChartContainerWidth - itemWidth)
            .y(40).itemHeight(50).gap(40).itemWidth(itemWidth));
