...
(async () => {
  ...
  let data = await fetchJsonFromUrlAsync(STATBANK_BASE_URL + TABLE_CODE)
  let dataset = JSONstat(data).Dataset(0)
  // let json = getTableMetadata(data)
  // console.log('json')
  // console.log(json)
  // console.log(json.valueslength)
  let STAT = 'Gross Value Added (GVA) per person at Basic Prices (Euro)'

  let colNames = dataset.id
  let table = dataset.toTable({ type: 'arrobj' })
  let tableFiltered = dataset.toTable(
     { type: 'arrobj' },
     (d, i) => {
       if ((d.Region === 'Dublin'
  ...
  // let out = dataset.Dimension(colNames[2])
  ...
  d3.csv('../data/Economy/RAA01.csv').then(csvData => {
    console.log(csvData[0])

    let columnNames = csvData.columns.slice(1)
    ...
    const GVA = new MultiLineChart(gvaContent)
    GVA.drawChart()
