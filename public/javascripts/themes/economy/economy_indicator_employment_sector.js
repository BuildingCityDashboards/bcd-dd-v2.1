...
Promise.all([
  ...
  .then(data => {
    ...
    const employmentSectorData = data[0]
    ...
    if (document.getElementById('chart-indicator-employment-sector')) {
      const broadSectorCols = employmentSectorColumns.slice(0, 5)

      const broadSectorData = employmentSectorData.map(d => {
      ...
      const employmentSectorChart = new StackedAreaChart(employmentSector)
    ...
    // console.log(employmentSectorColumns)
      ...
      const employmentServiceChart = new StackedAreaChart(employmentService)
